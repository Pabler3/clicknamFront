...
export class ModalEditRestauranteComponent implements OnInit {
  ...
  onFileChange(event: any): void {
    ...
    if (file.type.match(/image\/*/) == null) {
      this.editRestForm.get('foto_portada')!.setErrors({ 'invalid': true }); // establece el control como inválido
      this.editRestForm.get('foto_portada')!.markAsTouched(); // marca el control como tocado para mostrar el mensaje de error
      return;
    }
    ...
    reader.onload = () => {
      ...
      this.editRestForm.get('foto_portada')!.updateValueAndValidity(); // actualizamos el estado de validez de foto_portada después de establecer el nuevo valor
    };
  }
